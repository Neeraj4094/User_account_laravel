...
<body>
    <div class="grid grid-cols-12 grid-rows-6 w-full h-full bg-slate-950 text-slate-200 gap-1">
        <aside class=" row-span-6 col-span-1 border-r grid border-slate-500 h-full sm:hidden lg:block">
            <x-app />
        </aside>
        <main class="row-span-6 col-span-11  sm:col-span-12 lg:col-span-11 ">
            <div class="flex justify-between items-center border-b-2 border-slate-600 py-2 px-6">
                <p class="font-medium text-lg">Welcome Admin, {{ $name }}<span class="font-bold">
                        
                    </span></p>
                <div class="w-10 h-10 rounded-full border">
                    <?php
                    $col = '<svg class="w-full h-full object-cover text-slate-600" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 512"><!-- Font Awesome Free 5.15.4 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) --><path d="M248 8C111 8 0 119 0 256s111 248 248 248 248-111 248-248S385 8 248 8zm0 96c48.6 0 88 39.4 88 88s-39.4 88-88 88-88-39.4-88-88 39.4-88 88-88zm0 344c-58.7 0-111.3-26.6-146.5-68.2 18.8-35.4 55.6-59.8 98.5-59.8 2.4 0 4.8.4 7.1 1.1 13 4.2 26.6 6.9 40.9 6.9 14.3 0 28-2.7 40.9-6.9 2.3-.7 4.7-1.1 7.1-1.1 42.9 0 79.7 24.4 98.5 59.8C359.3 421.4 306.7 448 248 448z"></path></svg>';
                    echo "$col<br>";
            ...
            <div class="p-2 px-6">
                ...
                <div class="flex items-center justify-between ">
                    <div class="flex items-center relative">
                        <form action="{{url('/dashboard')}}" method="get" class="flex items-center gap-1 relative">
                            <input type="search" name="search" id="search"
                                class="border border-slate-500 shadow rounded-lg outline-none p-2 px-3 w-96 bg-slate-800 text-white" placeholder="Search...">
                            <button type="submit" class="p-2 pt-3 bg-slate-800 border border-slate-500 rounded-r-lg absolute right-0 top-0">
                    ...
                    <div class="flex gap-2 items-center">
                        <a href="{{url('/register')}}"
                            class=" uppercase px-4 py-2 bg-blue-700 text-white rounded-lg"><span
                                class="font-bold text-xl">+</span> Add User</a>
                        <a href="{{url('/trash_data')}}"
                            class=" uppercase px-4 py-2 bg-blue-700 text-white rounded-lg"><span
                                class="font-bold text-xl">+</span> Trash data</a>
                    </div>
                </div>
            </div>
            <div class="pl-2 border-t bg-slate-900 border-slate-500">
                <div class="px-4 h-96 pt-1 space-y-3 overflow-y-scroll">
                    @foreach ($userdata as $user_data)
                        ...
                        <!-- {{print_r(session()->all())}}  -->
                            <div
                                class="gap-2 py-2 mt-2 rounded-md border bg-slate-800 border-slate-700 w-full shadow shadow-slate-800 ">
                                <div class=" flex justify-between items-center p-2 gap-4 w-full ">
                                    ...
                                    <div class="flex items-center justify-between gap-6">
                                        <p class=" bg-blue-700 text-white px-3 py-1 rounded-lg">
                                            {{$status}}
                                        </p>
                                        ...
                                        <!-- <form action="" method="post" id="update"> -->
                                            <a href="{{route('user.edit', ['id' => $user_data['id']])}}" data-toggle="tooltip" data-placement="top" title="Edit"
                                                class="px-1 rounded-lg bg-slate-600 text-slate-200">
                                                <?php echo $edit ?>
                                            </a>
                                        <!-- </form> -->
                                        
                                        <button id="<?php echo 'showBookBtn_' . $checkid ?>"  data-toggle="tooltip" data-placement="top" title="Delete" class=" border-2 border-slate-500 px-4 py-1 rounded-md">
                                            <?php echo $delete ?>
                                        </button>
                                        <script>
                        ...
                        @endforeach
                        <div class="grid w-full items-center justify-center gap-2">
                            {{ $userdata->links() }}
                        </div>
